...
class CustomAdapterWords(private var list: ArrayList<String>, private val listener: CustomListener?
...
//        Log.d("MyTag","Get Item Count : "+list.size.toString())
    ...
    val dragInstance: DragListenerWord?
        get() = if (listener != null) {
            DragListenerWord(context,listener)
        } else {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (itemType == 0) {
            ...
            holder.frameLayout.setOnDragListener(listener?.let { DragListenerWord(context, it) })
